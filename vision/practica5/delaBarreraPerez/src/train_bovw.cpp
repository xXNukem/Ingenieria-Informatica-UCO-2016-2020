...
{
	int opc, kvecinos;
	cout << "1- DESCRIPTOR SIFT DENSO, 2- DESCRIPTOR SURF,3-DESCRIPTOR SIFT NORMAL otra cosa=SALIR" <<
	     endl;
	cin >> opc;
	cout << "Numero de K vecinos a utilizar: " << endl;
	cin >> kvecinos;
	...
	for (int trail = 0; trail < n_runsArg.getValue(); trail++)
	{
		...
		for (size_t c = 0, i = 0; c < train_samples.size(); ++c)
			for (size_t s = 0; s < train_samples[c].size(); ++s, ++i)
			{
				...
				cv::Mat bovw = compute_bovw(dict, keyws.rows, descriptors, kvecinos);
		...
		knnClassifier->setDefaultK(kvecinos);
		...
		for (size_t c = 0; c < test_samples.size(); ++c)
		{
			...
			for (size_t s = 0; s < test_samples[c].size(); ++s)
			{
				...
				else
				{
					...
					cv::Mat bovw = compute_bovw(dict, keyws.rows, descs, kvecinos);
					if (test_bovw.empty())
						test_bovw = bovw;
					else
					{
	...
	dictFile << "keywords" << keywords.getValue() << "descriptor" << opc << "ndesc" << ndesc.getValue()
	         << "kmeans" << kvecinos;

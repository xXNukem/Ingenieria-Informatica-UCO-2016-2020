...
cv::Mat createGaussianFilter(int r)
{
    ...
    cv::Mat gaussianFilter((2 * r + 1), (2 * r + 1), CV_32FC1,
                           Scalar(0)); //matriz del tamaño calculado con la formula de la practica
    ...
    for (int x = -r; x <= r; x++)
    {
        ...
        {
            XY = (x * x) + (y * y);
            gaussianFilter.at<float>(x + r,
                                     y + r) = (exp(-(XY * XY) / s)) / (M_PI * s); //Aplicamos la formula del filtro gausiano para meter los datos en la matriz del filtro
            sum += gaussianFilter.at<float>(x + r,
                                            y + r); //Vamos acumulando todo para luego hacer una normalizacion
        }
    }

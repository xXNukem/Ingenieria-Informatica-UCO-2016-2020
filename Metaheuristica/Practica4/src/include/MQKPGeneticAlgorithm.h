...
public:
	...
	virtual void run(MQKPStopCondition &stopCondition) {
		...
		while (stopCondition.reached() == false) {
			...
			//Almacenar la media de la población actual y la mejor solución (OLD)
			...
			_bestPerIterations.push_back(
					_population.at(bestSolIndex)->getFitness());
			if(MQKPEvaluator::compare(_bestPerIterations.back(), _bestSolution -> getFitness()) > 0)
		...
		//Almacenar la media de la poblaciónfinal y la mejor solución (OLD)
		/*_popMeanResults.push_back(computeMeanFitness(_population));
		_bestPerIterations.push_back(
				_population.at(indexBest(_population))->getFitness());*/

...
class MQKPAntColonyOpt : public MQKPMetaheuristic {
        ...
        void createAlternatives(
            ...
            for (auto indexObj : _objectsLeft) {
                ...
                for (unsigned j = 1;
                    ...
                    double significance = pow(density, beta) * pow((*phMatrix[indexObj])[j], alpha);
    ...
    public:
        ...
        void resetSolution() {
             ...
             */
            MQKPInstance *instance = _colony->_instance;
            for (auto indexObject = 0; indexObject < instance->getNumObjs(); indexObject++) {
                _sol->putObjectIn(indexObject, 0);
                _objectsLeft.insert(indexObject);
            }
            _sol->setFitness(0.0);
        }
...
protected:
	...
	void localUpdate(MQKPObjectAssignmentOperation &op) {
		double oldPh = _phMatrix.at(op.getObj())->at(op.getKnapsack());
		double newPh = (1 - _evaporation)*oldPh + _evaporation*_initTau;

		_phMatrix.at(op.getObj())->at(op.getKnapsack()) = newPh;
	}
	...
	void iterate() {
		...
		for (unsigned i = 0; i < numObjs; i++) {
			oldPh = _phMatrix.at(i)->at(_bestSolution->whereIsObject(i));
			newPh = (1 - _evaporation)*oldPh + _evaporation*fitness;

			_phMatrix.at(i)->at(_bestSolution->whereIsObject(i)) = newPh;
		}
	}

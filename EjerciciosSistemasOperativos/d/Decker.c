...
void *Proceso1(void *p)
{
	...
	for (i = 0; i < ITER; i++)
	{
		...
		{
			if (TurnoProceso==SEGUNDO)/*Si le toca entrar a P2*/
			{
				P1QuiereEntrar = FALSE;
				while (TurnoProceso==SEGUNDO);/*Espero mientras le toque entrar a P2*/
				P1QuiereEntrar = TRUE;	
			}
		}
		//Seccion critica
...
void *Proceso2(void *p)
{
	...
	for (i = 0; i < ITER; i++)
	{
		...
		while (P1QuiereEntrar)
			if (TurnoProceso==PRIMERO)/*Si le toca entrar a P1*/
			{
				P2QuiereEntrar = FALSE;
				while (TurnoProceso == PRIMERO);/*Espero mientras le toque entrar a P1*/
				P2QuiereEntrar = TRUE;	
			}
		//Seccion critica
		counter++;

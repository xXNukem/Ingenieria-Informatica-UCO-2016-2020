...
void ejecutarAlgoritmo(int opcion)
{
	...
	int repeticiones=0;
		...
		if(opcion==1)
		{
			...
			resolverSistemaEcuaciones(m.getMatrizCoeficientes(), m.getMatrizIndependientes(), m.getMatrizCoeficientes().size(), matrizSoluciones);
			...
			std::cout<<BIGREEN<<"Ecuacion: "<<BIBLUE<<matrizSoluciones[0][0]<<BIYELLOW<<"+"<<BIBLUE<<matrizSoluciones[1][0]<<BIYELLOW<<"*2^n"<<RESET<<std::endl;
		...
		if(opcion==2)
		{
			...
			cD=getCoeficienteDeterminacion(t_practico, t_teorico);
			std::cout<<BIBLUE<<"Coeficiente de determinacion: "<<BIYELLOW<<cD<<RESET<<std::endl;
			std::cout<<BIGREEN<<"Ecuacion: "<<BIBLUE<<matrizSoluciones[0][0]<<"+"<<matrizSoluciones[1][0]<<BIYELLOW<<"*n+"<<BIBLUE<<matrizSoluciones[2][0]<<BIYELLOW<<"*n²"<<RESET<<std::endl;
			t_teorico=calcularTiempoTeorico(matrizSoluciones,n,opcion);
			grabarFichero(n,t_practico,t_teorico);
			calcularDias(matrizSoluciones,opcion);			
		}
		...
		if(opcion==4)
		{
			...
			std::cin>>o;
				if(o=="m")
				{
					...
					std::cout<<BIGREEN<<"Ejecutando las torres de hanoi de "<<BIBLUE<<minDiscos<<BIGREEN<<" hasta "<<BIBLUE<<discos<<BIGREEN" discos "<<RESET<<std::endl;
					std::cout<<BIRED<<"Computando... ESPERA POR FAVOR"<<RESET<<std::endl;
					...
					std::cout<<BIGREEN<<"COMPUTADO!"<<RESET<<std::endl;
					std::cout<<BIGREEN<<"Se han llevado a cabo "<<BIBLUE<<movimientos<<BIGREEN" movimientos "<<RESET<<std::endl;
					m.setSistema(naux,t_practico,2,2);
					matrizSoluciones= std::vector<std::vector<double> >(2, std::vector< double >(1));
					resolverSistemaEcuaciones(m.getMatrizCoeficientes(), m.getMatrizIndependientes(), m.getMatrizCoeficientes().size(), matrizSoluciones);
					std::cout<<BIGREEN<<"SOLUCIONES"<<std::endl<<RESET;
...
std::vector<float> calcularTiempoTeorico(std::vector<std::vector<double> > & _matrizSoluciones, std::vector<double> & n, int & opcion)
{
	...
	float tiempo;
		...
		for(int i=0;i<n.size();i++)
		{
			...
			{ 
				tiempo=_matrizSoluciones[0][0]+_matrizSoluciones[1][0]*n[i]+_matrizSoluciones[2][0]*pow(n[i],2);
				t_teorico.push_back(tiempo);

...
void ed::Grafo::setCoste(float c)
{
	_coste=c;
...
void ed::Grafo::insertarMatriz(int x, int y,float distancia)
{
	_matriz[x][y]=distancia;
}
...
{
	int vL=v.getLabel();//Para no estar escribiendo getLabel y getNumVertices todo el rato que es muy cansino
...
float ed::Grafo::getMatriz(int i,int j)const
{
	return _matriz[i][j];
}
...
ed::Grafo ed::Grafo::operator=(ed::Grafo const & g)
{
	_matriz=g._matriz;
	_v=g._v;
	_coste=g._coste;
